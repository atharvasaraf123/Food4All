...
class OngoingDonation extends StatefulWidget {
  @override
  _OngoingDonationState createState() => _OngoingDonationState();
}
...
class _OngoingDonationState extends State<OngoingDonation> {
  ...
  getOngoingDonation()async{
    ...
    await donCol.get().then((value){
      ...
      for(int i=0;i<list.length;i++){
        if( list[i]['uid']==uid &&!isBefore(list[i]['dateTime'])){
          don.add(list[i]);
        }
      }
      setState(() {
  ...
  Widget build(BuildContext context) {
    return don.length==0?Scaffold(
        appBar: AppBar(title: Text('Ongoing Donations',style: TextStyle(
          fontWeight: FontWeight.bold,
          fontSize: 20,
          fontFamily: 'MontserratSemi',
        ),),),
        body: Center(child: Text('No Ongoing donations',style: TextStyle(
          fontWeight: FontWeight.bold,
          fontSize: 20,
          fontFamily: 'MontserratSemi',
        ),))):Scaffold(
      appBar: AppBar(title: Text('Ongoing Donations'),),
